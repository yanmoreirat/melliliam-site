...
DECLARE
  ...
  v_coupon_id UUID DEFAULT NULL;
  v_coupon_code TEXT DEFAULT NULL;
  v_coupon_type TEXT DEFAULT NULL;
  v_coupon_value NUMERIC(10,2) DEFAULT NULL;
  v_item RECORD;
  v_product RECORD;
...
BEGIN
  ...
  IF p_coupon_code IS NOT NULL AND TRIM(p_coupon_code) <> '' THEN
    SELECT id, code, type, value INTO v_coupon_id, v_coupon_code, v_coupon_type, v_coupon_value
    FROM public.coupons
  ...
  IF v_coupon_id IS NOT NULL THEN
    IF v_coupon_type = 'percent' THEN
      v_coupon_discount := ROUND(((v_subtotal - v_discount_total) * (v_coupon_value / 100)), 2);
    ELSE
      v_coupon_discount := v_coupon_value;
    END IF;
